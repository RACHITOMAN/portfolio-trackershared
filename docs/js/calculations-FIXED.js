...
function isValidTransaction(t) {
  ...
  // SELL transactions can have negative shares (this is correct!)
  // Only reject if shares is exactly 0 (unless it's a cash dividend)
  if (!isCashDividend && t.shares === 0) {
    return false;
  }
  
  // Also reject if shares is null/undefined
  if (!isCashDividend && (t.shares === null || t.shares === undefined)) {
    return false;
  }
  ...
  // Premiums are always valid (can have any price, including negative)
  if (t.type === 'premium') {
    return true;
  }
